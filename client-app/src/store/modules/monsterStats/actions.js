...
export default {
  async monsterAction({ dispatch, getters }) {
    var randomBattleAction = getRandomValue(
      0,
      getters.monsterBattleActionsList.length - 1
    );
    ...
    await dispatch("endMonsterTurn", {
      by: Characters.MONSTER,
      type: actionName,
      value: actionValue,
    });
  },
  async endMonsterTurn({ commit, dispatch }, payload) {
    ...
    await dispatch("battleMessages/battleMessageAnimation", null, {
      ...
      await dispatch(
        ...
        { increase: false, value: payload.value },
        { root: true }
      ).then(() => {
  ...
  monsterHealthBarChangeAnimation({ commit, getters }, payload) {
    return new Promise((resolve) => {
      ...
      var newHealth = payload.increase
        ? getters.currentMonsterHealth + payload.value
        : getters.currentMonsterHealth - payload.value;
  ...
  setCurrentMonster({ commit }, payload) {
    commit("setCurrentMonster", { value: payload.value });
  },
};
